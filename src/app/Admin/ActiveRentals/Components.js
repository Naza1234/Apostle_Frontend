...
import { GetRentalActive, SendMessageHandler } from "@/app/Adminapicalls";
...
const Components = () => {
  ...
  const loadUser = async (page) => {
    try {
      const result = await GetRentalActive(page);
  ...
  return (
    <section className="history">
      ...
      <h2 className="info">
        Please, there is no Active rental.
      </h2>
          ...
          {apiResponse.map((rental, index) => {
            ...
            return (
              <li key={index} >
                ...
                <span>  {Math.floor((now - new Date(DateRented)) / (1000 * 60 * 60))} hrs</span>
