...
import { GetRentalHistory ,SendMessageHandler  } from "@/app/Adminapicalls";
...
const Components = () => {
  ...
  const loadUser = async (page) => {
    try {
      const result = await GetRentalHistory(page);
          ...
          <span>Payment</span>
            ...
            return (
              <li key={index} >
                ...
                <span>
                  {!DateReturned 
                    ? `${Math.floor((new Date() - new Date(DateRented)) / (1000 * 60 * 60))} hrs`
                    : `${RentalDuration} hrs`}
                </span>
